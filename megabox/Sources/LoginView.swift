...
struct LoginView: View {
    ...
    @AppStorage("saved_pwd") private var savedPwd: String = ""
    ...
    var body: some View {
        VStack {
            ...
            VStack {
                ...
                SecureField("비밀번호", text: $viewModel.model.pwd)
                    .padding(.horizontal, 16)
                Divider()
                    .padding(.horizontal, 16)
                ...
                Button(action: {
                    self.savedId = viewModel.model.id
                    self.savedPwd = viewModel.model.pwd
                }) {
                    // 버튼에 표시될 내용
                    Text("로그인")
                        .font(.bold18)
